...
const teaCategories = (
  state: { loading: boolean; categories?: Array<TeaCategory>; error?: Error } = { loading: false, categories: [] },
  action: any
) => {
  switch (action.type) {
    ...
    case TeaCategoryActionTypes.updateSuccess:
      ...
      return {
        ...state,
        loading: false,
        categories:[...state.categories!.slice(0, idx), action.payload, ...state.categories!.slice(idx + 1)]
      };
